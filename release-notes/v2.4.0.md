...
### Muxing Options
- **Mux-only passes for already-efficient files** -- new `MuxMode` (Off / Audio / Subtitles / Both) and `MuxScope` (TargetMatchOnly / AllFiles) let you re-mux audio and subtitle tracks without re-encoding video. Files that already meet the bitrate target can be remuxed to standardize audio codec or inject OCR'd subtitles in seconds instead of being re-encoded.
